...
export class MarcheeComponent implements OnInit {
  ...
  public addBC(addBCForm:NgForm):void{
    ...
    let tot_delais = 0
    this.newMarchee.listeBondeCommandes.map((e: BondeCommande)=>{
      tot_delais += e.delais
    })
    tot_delais+=addBCForm.value.delais
    console.log(tot_delais)
    console.log(this.newMarchee.delais)
    if(!this.validerDelaiBC(tot_delais, this.newMarchee)){
    ...
    if(this.validerDelaiBC(tot_delais, this.newMarchee)&&this.validerDelaiBC(addBCForm.value.delais,this.newMarchee)&&!this.testExistCodeBC(this.codeNewBC,this.newMarchee.listeBondeCommandes)){
      this.organService.getOrganisationbyCode(this.codeOrganisation).subscribe({
        next: (response:Organisation) => {
          ...
          console.log("codeOrganisation"+this.codeOrganisation);
          document.getElementById('addBCModal')?.click();
          let newBC = new BondeCommande();
          newBC.idOrganisation=this.entreprise.id;
          newBC.nomOrganisation=this.entreprise.nom;
          newBC.codebc=this.codeNewBC;
          newBC.montant=this.montantNewBC;
          newBC.delais=this.delaisNewBC;
          newBC.dateDebutNewBC=this.dateDebutNewBC;
          console.log(newBC)
...
public testerCodeOrganisation(code:string):boolean{
...
public EnregisterBC(idMarchee:number,bc:BondeCommande):void{
  this.bondeCommandeService.addBondeCommande(idMarchee,bc.idOrganisation,bc).subscribe({
    next: (response:BondeCommande) => {
      ...
      for (let i = 0; i < bc.listeArticles.length; i++){
        this.articleService.getArticlebyCode(bc.listeArticles[i].code).subscribe(
          (res: any)=>{
            let art = bc.listeArticles[i]
            art.id = res.id
            this.EnregisterArticle(idBondeCommande,art)
          }
        )
      }
    },
...
public EnregisterArticle(idBC:number,article:Article):void{
 ...
 console.log(article)
  this.articleUtiliseeService.addArticleUtilisee(articleUtiliseeId.bondecommande_id,article.id,artcUtilisee).subscribe({
